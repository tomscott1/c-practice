...
void merge(int array[], int left, int right, int length) {
  ...
  while ((position1 < ((left + right)/2)+1) && (position2 < right + 1)) {
    ...
    if (position2 == right + 1) {
      while ((position1<((left+right)/2)+1)) {
        array_tmp[current++] = array[position1];
        position1++;
      };
    } else {
      while ((position2 < right + 1)) {
        array_tmp[current++] = array[position2];
        position2++;
      };
    }

    for (current = left; current < right - left + 1;current++) {
      array[current] = array_tmp[current];
    }
  }
}
